...
public class CheaterPlayerStrategy extends PlayerStrategy{
    ...
    /**
     * A cheater computer playerstrategy whoseissueOrder()
     * method conquers all the immediate neighboring enemy countries, and then doubles the number of armies on 
     * its countries that have enemy neighbors.
     *  Note that in order to achieve this, the cheater’s strategy implementation will still be called when 
     * the issueOrder() method, but will not end up creating orders, 
     * but rather implement the above-stated behavior by directly affecting 
     * the map during the order creation phase.
     */
    @Override
